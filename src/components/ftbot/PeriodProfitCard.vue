...
const yearlyProjectedEndCompound = computed(() => {
  if (todayDayOfYear < 30 || yearlyCurrentProfit.value <= 0 || !profit.value?.profit_closed_ratio)
    return yearlyProjectedEnd.value;
  ...
  const projected =
    capitalNow * Math.pow(1 + dailyRate, daysInYear - todayDayOfYear) - capitalInitial;
...
const isRecentBelowAvg = computed(() => {
  ...
  const simpleAnnualized =
    profit.value.profit_closed_ratio * (365 / Math.max(tradingPeriodDays.value, 1));
    ...
    <template v-else>
      ...
      <div class="mb-3 pb-3 border-b border-black/5 dark:border-white/5">
        <div class="flex items-center justify-between">
          <div>
            <div
              class="text-[0.8rem] text-gray-600 dark:text-gray-500 mb-0.5"
              v-tooltip.top="t('tooltips.weeklyAvg')"
            >
              {{ t('periodProfitCard.weeklyAvg') }}
            </div>
            <div class="text-2xl font-bold" :class="profitColor(weeklyAvgProfit)">
              {{
                weeklyAvgProfit !== undefined
                  ? formatPriceCurrency(weeklyAvgProfit, currency, 2)
                  : 'N/A'
              }}
            </div>
            <div
              v-if="weeklyAvgRatio !== undefined"
              class="text-[0.8rem]"
              :class="profitColor(weeklyAvgRatio)"
            >
          ...
          <div class="text-right">
            <div
              class="text-[0.8rem] text-gray-600 dark:text-gray-500 mb-0.5"
              v-tooltip.top="t('tooltips.monthlyAvg')"
            >
              {{ t('periodProfitCard.monthlyAvg') }}
            </div>
            <div class="text-lg font-bold" :class="profitColor(monthlyAvgProfit)">
              {{
                monthlyAvgProfit !== undefined
                  ? formatPriceCurrency(monthlyAvgProfit, currency, 2)
                  : 'N/A'
              }}
            </div>
            <div
              v-if="monthlyAvgRatio !== undefined"
              class="text-[0.8rem]"
              :class="profitColor(monthlyAvgRatio)"
            >
      ...
      <div class="mb-3 pb-3 border-b border-black/5 dark:border-white/5">
        ...
        <div class="space-y-0.5">
          <div class="stat-row">
            <span class="stat-label" v-tooltip.top="t('tooltips.dailyAvg')">{{
              t('periodProfitCard.dailyAvg')
            }}</span>
            <span class="stat-value" :class="profitColor(dailyAvgProfit)">
              {{
                dailyAvgProfit !== undefined
                  ? formatPriceCurrency(dailyAvgProfit, currency, 2)
                  : 'N/A'
              }}
            </span>
          </div>
          <div v-if="annualizedReturn !== undefined" class="stat-row">
            <span class="stat-label" v-tooltip.top="t('tooltips.annualizedReturn')">{{
              t('periodProfitCard.annualizedReturn')
            }}</span>
          ...
          <div
            v-if="annualizedCompound !== undefined && annualizedCompound !== annualizedReturn"
            class="stat-row"
          >
            <span class="stat-label" v-tooltip.top="t('tooltips.annualizedCompound')">{{
              t('periodProfitCard.annualizedCompound')
            }}</span>
          ...
          <div v-if="profit.cagr !== undefined && Math.abs(profit.cagr) < 100" class="stat-row">
            <span class="stat-label" v-tooltip.top="t('tooltips.cagr')">{{
              t('profit.cagr')
            }}</span>
      ...
      <div class="mb-3 pb-3 border-b border-black/5 dark:border-white/5">
        ...
        <div class="text-[0.8rem] text-gray-600 mb-2">
          {{ t('periodProfitCard.projectionsDisclaimer') }}
        </div>
        <div class="grid grid-cols-3 gap-2">
          <div class="metric-cell">
            <div
              class="text-[0.85rem] text-gray-600 dark:text-gray-500 mb-0.5"
              v-tooltip.top="t('tooltips.projection1m')"
            >
              1 {{ t('periodProfitCard.month') }}
            </div>
          ...
          <div class="metric-cell">
            <div
              class="text-[0.85rem] text-gray-600 dark:text-gray-500 mb-0.5"
              v-tooltip.top="t('tooltips.projection3m')"
            >
              3 {{ t('periodProfitCard.months') }}
            </div>
          ...
          <div class="metric-cell">
            <div
              class="text-[0.85rem] text-gray-600 dark:text-gray-500 mb-0.5"
              v-tooltip.top="t('tooltips.projection1y')"
            >
              1 {{ t('periodProfitCard.year') }}
            </div>
      ...
      <div class="mb-3 pb-3 border-b border-black/5 dark:border-white/5">
        <div class="section-header">
          ...
          <span
            class="ml-auto text-[0.85rem]"
            :class="statusColor(monthlyTarget, monthlyProjectedEnd)"
          >
            {{ monthlyStatus }}
          </span>
        </div>
        ...
        <div class="mb-2">
          <div class="flex justify-between text-[0.8rem] text-gray-600 dark:text-gray-500 mb-1">
            <span
              >{{ t('periodProfitCard.actual') }}:
              {{ formatPriceCurrency(monthlyCurrentProfit, currency, 2) }}</span
            >
            <span
              >{{ t('periodProfitCard.target') }}:
              {{ formatPriceCurrency(monthlyTarget, currency, 2) }}</span
            >
          </div>
          <div class="progress-track">
            <div class="progress-time" :style="{ width: monthProgress + '%' }" />
            <div
        ...
        <svg :width="320" :height="120" class="projection-chart">
          <!-- Background grid -->
          <line
            :x1="monthlyChart.xStart"
            :y1="monthlyChart.zeroY"
            :x2="monthlyChart.xEnd"
            :y2="monthlyChart.zeroY"
          ...
          <line
            :x1="monthlyChart.xStart"
            :y1="monthlyChart.targetY"
            :x2="monthlyChart.xEnd"
            :y2="monthlyChart.targetY"
            stroke="rgba(156,163,175,0.4)"
            stroke-width="1"
            stroke-dasharray="3,3"
          />
          <text
            :x="monthlyChart.xEnd - 2"
            :y="monthlyChart.targetY - 3"
          ...
          <!-- Today vertical line -->
          <line
            :x1="monthlyChart.todayX"
            y1="14"
            :x2="monthlyChart.todayX"
            y2="104"
          ...
          />
          <text
            :x="monthlyChart.todayX"
            y="11"
            fill="rgba(34,211,238,0.7)"
            font-size="9"
            text-anchor="middle"
          >
            {{ t('periodProfitCard.today') }}
          </text>
          <!-- X-axis labels -->
          <text
            :x="monthlyChart.xStart"
            y="116"
        ...
        <div class="flex justify-between text-[0.8rem] text-gray-600 dark:text-gray-500 mt-1">
          <span
            >{{ t('periodProfitCard.projection') }}:
            {{ formatPriceCurrency(monthlyProjectedEnd, currency, 2) }}</span
          >
        </div>
      </div>
      ...
      <div class="mb-3 pb-3 border-b border-black/5 dark:border-white/5">
        <div class="section-header">
          ...
          <span
            class="ml-auto text-[0.85rem]"
            :class="statusColor(yearlyTarget, yearlyProjectedEnd)"
          >
            {{ yearlyStatus }}
          </span>
        </div>
        ...
        <svg :width="320" :height="120" class="projection-chart">
          <!-- Background grid -->
          <line
            :x1="yearlyChart.xStart"
            :y1="yearlyChart.zeroY"
            :x2="yearlyChart.xEnd"
            :y2="yearlyChart.zeroY"
            stroke="rgba(255,255,255,0.06)"
            stroke-width="1"
          />
          <!-- Target horizontal line -->
          <line
            :x1="yearlyChart.xStart"
            :y1="yearlyChart.targetY"
            :x2="yearlyChart.xEnd"
            :y2="yearlyChart.targetY"
            stroke="rgba(156,163,175,0.4)"
            stroke-width="1"
            stroke-dasharray="3,3"
          />
          <text
            :x="yearlyChart.xEnd - 2"
            :y="yearlyChart.targetY - 3"
            fill="rgba(156,163,175,0.5)"
            font-size="9"
            text-anchor="end"
          >
          ...
          <!-- Today vertical line -->
          <line
            :x1="yearlyChart.todayX"
            y1="14"
            :x2="yearlyChart.todayX"
            y2="104"
            stroke="rgba(34,211,238,0.5)"
            stroke-width="1"
            stroke-dasharray="2,2"
        ...
        <div class="flex justify-between text-[0.8rem] text-gray-600 dark:text-gray-500 mt-1">
          <span
            >{{ t('periodProfitCard.actual') }}:
            {{ formatPriceCurrency(yearlyCurrentProfit, currency, 2) }}</span
          >
          <span
            >{{ t('periodProfitCard.projection') }}:
            {{ formatPriceCurrency(yearlyProjectedEnd, currency, 0) }}</span
          >
        </div>
        <div
          v-if="yearlyProjectedEndCompound !== yearlyProjectedEnd"
          class="flex justify-between text-[0.8rem] mt-0.5"
        >
          <span class="text-purple-400/70">{{ t('periodProfitCard.compoundProjection') }}</span>
          <span class="text-purple-400 font-bold">{{
            formatPriceCurrency(yearlyProjectedEndCompound, currency, 0)
          }}</span>
        </div>
      </div>
      ...
      <div>
        <div class="stat-row">
          <span class="stat-label" v-tooltip="t('tooltips.tradingPeriod')">{{
            t('periodProfitCard.tradingPeriod')
          }}</span>
          <span class="stat-value"
            >{{ formatNumber(tradingPeriodDays, 0) }} {{ t('periodProfitCard.days') }}</span
          >
        </div>
        <div class="stat-row">
          <span class="stat-label" v-tooltip="t('tooltips.realizedProfit')">{{
            t('periodProfitCard.totalRealized')
          }}</span>
