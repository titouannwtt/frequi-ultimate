...
const { tradingMode, hasMultipleModes, isBotInMode, restorePersistedTradingMode } =
  useTradingModeFilter('logConsole');
...
const { filtersChanged, saveCurrentAsDefault, loadDefaults } = useWidgetDefaults(
  ...
  (d) => {
    ...
    if (d.activeTimeWindow !== undefined) {
      activeTimeWindow.value = d.activeTimeWindow as number;
      setTimeWindow(d.activeTimeWindow as number);
    }
...
onMounted(() => {
  tickInterval = setInterval(() => tick.value++, 10_000);
});
...
const displayEntries = computed(() => {
  ...
  if (hideWebSocket.value) {
    result = result.filter(
      (e) =>
        !e.module.toLowerCase().includes('uvicorn') &&
...
watch(
  [selectedCustomTags, selectedCurrencies],
  () => {
    // If custom tag or currency filter active, restrict bot filter
    const tagBots =
      selectedCustomTags.value.size > 0
        ? new Set(
            Object.entries(compStore.botCustomTags)
    ...
    const curBots =
      selectedCurrencies.value.size > 0
        ? new Set(
            Object.entries(botStore.allBotState)
...
<template>
  <div ref="widgetEl" class="flex flex-col h-full overflow-hidden text-xs text-left">
    ...
    <div class="flex items-center gap-2 px-3 py-1.5 border-b border-surface-700/50 flex-shrink-0">
      ...
      <span
        v-if="summaryCounts.total === 0 && logStore.initialLoadDone"
      ...
      <span class="ml-auto flex items-center gap-2 text-[10px] text-surface-500">
        ...
        <template v-else>
          <span
            v-if="logStore.fetching"
            class="animate-spin inline-block w-3 h-3 border border-blue-400 border-t-transparent rounded-full"
          />
    ...
    <!-- ═══ FILTER BAR ═══ -->
    <div
      class="flex flex-wrap items-center gap-1.5 px-3 py-1 border-b border-surface-700/30 flex-shrink-0"
    >
      <!-- Severity toggles -->
      <button
      ...
      <button
        ...
        :class="
          activeTimeWindow === tw.hours
            ? 'bg-blue-500/20 text-blue-400 font-bold'
            : 'text-surface-400 hover:text-surface-200'
        "
        :style="{
          border: `1px solid ${activeTimeWindow === tw.hours ? 'rgba(59,130,246,0.3)' : 'rgba(255,255,255,0.04)'}`,
        }"
      ...
      >
        Filters
        <span v-if="activeFilterCount > 0" class="text-[8px] font-bold text-purple-400 ml-0.5"
    ...
    <div
      ...
      class="px-3 py-1.5 border-b border-surface-700/30 flex-shrink-0 space-y-1.5"
    ...
    >
      <div class="flex flex-wrap gap-3">
        ...
        <div class="space-y-0.5">
          ...
          <div class="flex flex-wrap gap-0.5">
            <button
              ...
              :class="
                filters.botIds.size === 0 || filters.botIds.has(bot.id)
        ...
        <div v-if="availableExchanges.length > 1" class="space-y-0.5">
          ...
          <div class="flex flex-wrap gap-0.5">
            <button
              ...
              :class="
                filters.exchanges.size === 0 || filters.exchanges.has(ex)
                  ? 'text-surface-200 bg-surface-700/50'
                  : 'text-surface-500 bg-surface-800/30 opacity-50'
              "
        ...
        <div class="space-y-0.5">
          ...
          <div class="flex gap-0.5">
            <button
              v-for="mode in ['all', 'live', 'dry'] as const"
              :key="mode"
              class="px-1.5 py-0.5 rounded text-[9px] cursor-pointer capitalize"
              :class="
                filters.botMode === mode ? 'text-blue-400 bg-blue-500/15' : 'text-surface-400'
              "
        ...
        <div class="space-y-0.5">
          ...
          <div class="flex gap-0.5">
            <button
              v-for="mt in ['all', 'spot', 'futures'] as const"
              :key="mt"
              class="px-1.5 py-0.5 rounded text-[9px] cursor-pointer capitalize"
              :class="
                filters.marketType === mt ? 'text-blue-400 bg-blue-500/15' : 'text-surface-400'
              "
        ...
        <div>
          ...
          <div class="flex flex-wrap gap-0.5">
            <button
              ...
              :class="
                selectedCurrencies.size === 0 || selectedCurrencies.has(cur)
                  ? 'text-surface-200 bg-surface-700/50'
                  : 'text-surface-500 bg-surface-800/30 opacity-50'
              "
      ...
      <div v-if="availableCustomTags.length > 0">
        <div class="text-[9px] text-surface-500 uppercase tracking-wider mb-0.5">
          Tags ({{ availableCustomTags.length }})
        </div>
        <div
          class="flex flex-wrap gap-0.5"
          :class="availableCustomTags.length > 10 ? 'max-h-[50px] overflow-y-auto' : ''"
        >
          <button
            ...
            :style="{
              color:
                selectedCustomTags.size === 0 || selectedCustomTags.has(tag.id)
                  ? tag.color
                  : '#6b7280',
    ...
    <div class="flex-1 min-h-0 relative">
      ...
      <div ref="logScrollEl" class="h-full overflow-y-auto" @scroll="onScroll">
        <!-- Loading state -->
        <div
          v-if="!logStore.initialLoadDone"
          class="flex flex-col items-center justify-center h-full gap-2 text-surface-400"
        >
          <span
            class="animate-spin inline-block w-5 h-5 border-2 border-blue-400 border-t-transparent rounded-full"
          />
        ...
        <div
          v-else-if="summaryCounts.total === 0 && !logStore.fetching"
          class="flex flex-col items-center justify-center h-full gap-2"
        >
          <span class="text-2xl">&#x2705;</span>
          <span class="text-sm text-green-400 font-semibold"
            >All {{ totalBotCount }} bots running without issues</span
          >
        ...
        <template v-else-if="displayMode === 'grouped'">
          ...
          <div
            v-if="groupedByBot.length > 1"
            class="px-3 py-1 text-[10px] text-surface-400 border-b border-surface-700/30"
          >
            {{ groupedByBot.length }} bots with issues ·
            <span v-if="summaryCounts.critical" style="color: #ef4444"
              >{{ summaryCounts.critical }} critical</span
            >
            <span v-if="summaryCounts.error" style="color: #f97316"
              >{{ ' ' }}{{ summaryCounts.error }} error</span
            >
            <span v-if="summaryCounts.warning" style="color: #eab308"
              >{{ ' ' }}{{ summaryCounts.warning }} warning</span
            >
          </div>

          <div
            v-for="(botSection, secIdx) in groupedByBot"
            :key="botSection.botId"
            class="border-b border-surface-700/30"
          ...
          >
            ...
            >
              <span class="text-[10px] text-surface-500">{{
                collapsedBots.has(botSection.botId) ? '▸' : '▾'
              }}</span>
              <BotNameTruncated
                :name="botSection.botName"
                max-width="140px"
                class="font-semibold text-[11px] text-surface-200"
              />
              <button
              ...
              <span
                ...
                :style="{
                  color: sevColor(botSection.maxSeverity),
                  background: sevBg(botSection.maxSeverity),
                }"
              >
                {{ botSection.groups.reduce((s, g) => s + g.count, 0) }} issue{{
                  botSection.groups.reduce((s, g) => s + g.count, 0) > 1 ? 's' : ''
                }}
              </span>
            </div>
              ...
              >
                ...
                >
                  <span
                    ...
                    :style="{ background: sevColor(group.level) }"
                  />
                  <span
                  ...
                  <span
                    v-if="group.count > 1"
                    class="text-[10px] font-bold flex-shrink-0 px-1"
                    :style="{ color: sevColor(group.level) }"
                  >
                    ×{{ group.count }}
                  </span>
                  <button
                ...
                <div
                  v-if="!isCompact"
                  class="flex items-center gap-3 mt-0.5 ml-3.5 text-[9px] text-surface-500"
                >
                  <span v-if="group.count > 1"
                    >{{ relativeTime(group.last.timestamp) }} →
                    {{ relativeTime(group.first.timestamp) }}</span
                  >
                ...
                <!-- Expanded detail -->
                <div
                  v-if="expandedGroupKey === group.key && !isCompact"
                  class="ml-3.5 mt-1 mb-1 space-y-0.5"
                >
                  <div
                  ...
                  >
                    <span class="text-surface-500 flex-shrink-0 font-mono">{{
                      entry.timestampFormatted.slice(11)
                    }}</span>
          ...
          >
            +{{ groupedByBot.length - 5 }} more bots...
          </button>
          <button
          ...
          <!-- Bots without issues -->
          <div
            v-if="botsWithoutIssues > 0"
            class="px-3 py-2 text-center text-[11px] text-surface-500"
          >
            <span class="text-green-500">&#x2713;</span> {{ botsWithoutIssues }} bot{{
              botsWithoutIssues > 1 ? 's' : ''
            }}
            without issues
          </div>
          ...
          <div
            v-if="
              groupedByBot.length === 0 && summaryCounts.total === 0 && logStore.initialLoadDone
            "
            class="px-3 py-4 text-center text-[11px] text-surface-500"
          >
        ...
        <template v-else>
          ...
          >
            <span
              class="w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0"
              :style="{ background: sevColor(entry.level) }"
            />
            <span class="text-[10px] text-surface-500 font-mono flex-shrink-0 w-[50px]">{{
              entry.timestampFormatted.slice(11, 19)
            }}</span>
            <BotNameTruncated
              :name="entry.botName"
              max-width="80px"
              class="text-[10px] text-surface-400 flex-shrink-0"
            />
            <span
              class="text-[10px] text-surface-300 font-mono flex-1 break-all"
              :class="isCompact ? 'truncate' : ''"
              >{{ entry.message }}</span
            >
            <span v-if="!isCompact" class="text-[9px] text-surface-600 flex-shrink-0">{{
              shortModule(entry.module)
            }}</span>
          </div>
          <div
            v-if="displayEntries.length > 500"
            class="px-3 py-2 text-center text-[10px] text-surface-500"
          >
        ...
        <!-- ═══ UNREACHABLE BOTS ═══ -->
        <div
          v-if="logStore.unreachableBotCount > 0"
          class="border-t border-surface-700/50 px-3 py-1.5"
        >
          ...
          >
            <span
              class="w-2 h-2 rounded-full bg-gray-200 dark:bg-gray-500 animate-pulse flex-shrink-0"
            />
            <span class="text-surface-300">{{ status.botName }}</span>
            <span
              class="text-surface-600 text-[9px] ml-auto truncate max-w-[200px]"
