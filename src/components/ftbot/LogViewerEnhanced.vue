...
const highlightText = computed(
  () => (route.query.highlight as string) || (route.query.q as string) || '',
);
...
function levelBorderClass(level: string): string {
  switch (level) {
    case 'CRITICAL':
      return 'border-l-4 border-l-red-800 bg-red-900/30';
    case 'ERROR':
    ...
    default:
      return 'border-l-4 border-l-surface-400';
  }
}
...
<template>
  <div class="flex flex-col h-full w-full">
    <!-- ═══ TOOLBAR ═══ -->
    <div
      class="flex flex-wrap items-center gap-2 px-2 py-1.5 border-b border-surface-300 dark:border-surface-600"
    >
      <!-- Severity toggles -->
        ...
        <button
          ...
          :class="
            !filters.levels.has(level)
              ? 'bg-surface-200/50 dark:bg-surface-700/50 text-surface-500 dark:text-surface-400'
              : ''
          "
      ...
      >
        Bots
        <span v-if="activeFilterCount > 0" class="text-[9px] font-bold ml-0.5"
    ...
    <div
      ...
      class="px-3 py-1.5 border-b border-surface-300 dark:border-surface-600 flex flex-wrap gap-3"
    ...
    >
      ...
      <button
        ...
        class="text-[9px] text-red-400 hover:text-red-300 cursor-pointer self-end"
        @click="
          resetFilters();
          filters.levels = new Set(['CRITICAL', 'ERROR', 'WARNING', 'INFO']);
          filters.excludedModulePrefixes = [];
        "
      >
        Reset
    ...
    <div class="flex items-center gap-2 px-2 py-0.5 text-[10px] text-surface-400">
      ...
      <span v-if="summaryCounts.critical > 0" style="color: #ef4444" class="font-bold"
        >{{ summaryCounts.critical }} critical</span
      >
      <span v-if="summaryCounts.error > 0" style="color: #f97316" class="font-bold"
        >{{ summaryCounts.error }} error</span
      >
      <span v-if="summaryCounts.warning > 0" style="color: #eab308"
        >{{ summaryCounts.warning }} warning</span
      >
      <span class="ml-auto text-surface-500">{{ totalBotCount }} bots</span>
      <span v-if="logStore.unreachableBotCount > 0" class="text-red-400"
        >{{ logStore.unreachableBotCount }} offline</span
      >
    </div>
    ...
    <div ref="scrollContainer" class="flex-1 overflow-auto px-1 pb-4">
      ...
      <template v-else-if="displayMode === 'grouped'">
          ...
          <template v-if="!collapsedBots.has(botSection.botId)">
            ...
            >
              ...
              >
                ...
                />
                <span
                  class="font-mono text-[11px] text-surface-200 flex-1 break-all"
              ...
              <div v-if="expandedGroupKey === group.key" class="ml-3.5 mt-1 mb-1 space-y-0.5">
                <div
                  v-for="entry in group.entries.slice(0, 10)"
                  :key="entry.id"
                ...
                <div v-if="group.entries.length > 10" class="text-[9px] text-surface-500 px-1.5">
        ...
        <div
          v-if="groupedByBot.length === 0 && logStore.initialLoadDone"
          class="text-center text-surface-400 text-sm py-8"
        >
      ...
      <template v-else>
        <template v-if="compactMode">
          <div
            ...
            :class="[
              levelBorderClass(entry.level),
              highlightText && entry.message.toLowerCase().includes(highlightText.toLowerCase())
                ? 'ring-1 ring-yellow-500/50 bg-yellow-500/10'
                : '',
              logStore.newEntryIds[entry.id] ? 'log-entry-new' : '',
            ]"
          >
            <span class="text-surface-500 shrink-0 whitespace-nowrap">{{
              entry.timestampFormatted
            }}</span>
            <span class="shrink-0 w-16 text-center" :class="levelTextClass(entry.level)">{{
              entry.level
            }}</span>
            <span
              class="shrink-0 px-1 rounded text-[10px] bg-surface-700 text-blue-300 whitespace-nowrap"
              >{{ entry.botName }}</span
            >
            <span
              class="shrink-0 px-1 rounded text-[10px] bg-surface-200 dark:bg-surface-700 text-surface-500 dark:text-surface-300 whitespace-nowrap"
              >{{ shortModule(entry.module) }}</span
            >
            <span class="dark:text-gray-200 break-all" @click="copyMessage(entry.message)">{{
              entry.message
            }}</span>
          </div>
        </template>
        ...
        <template v-else>
          ...
          >
            <div class="flex items-center gap-2 mb-0.5">
              ...
              <span
                class="inline-block px-1.5 rounded text-[10px] font-bold"
                :class="levelTextClass(entry.level)"
                >{{ entry.level }}</span
              >
              <span class="inline-block px-1.5 rounded text-[10px] bg-surface-700 text-blue-300">{{
                entry.botName
              }}</span>
              <span
                class="inline-block px-1.5 rounded text-[10px] bg-surface-200 dark:bg-surface-700 text-surface-400"
                >{{ shortModule(entry.module) }}</span
              >
            </div>
            <div
              class="text-xs dark:text-gray-200 font-mono whitespace-pre-wrap break-all"
            ...
            <div
              ...
              class="mt-1 px-2 py-1 rounded text-[9px] font-mono text-red-300 whitespace-pre-wrap break-all"
              style="background: rgba(239, 68, 68, 0.08); max-height: 150px; overflow-y: auto"
            >
              {{ entry.exception }}
            </div>
          </div>
        ...
        <div
          v-if="displayEntries.length === 0 && logStore.initialLoadDone"
          class="text-center text-surface-400 text-sm py-8"
        >
          {{ t('logViewer.noEntries') }}
        </div>
