...
const currency = computed(
  () => props.stakeCurrency || (props.botState?.stake_currency as string) || 'USDC',
);
...
<template>
  <div class="glass-card text-xs" style="width: 560px">
    ...
    <div v-if="isOnline === false" class="flex flex-col items-center gap-3 py-4">
      ...
      <div class="text-center">
        ...
        <div class="text-xs text-gray-600 dark:text-gray-400 mb-2">
          {{ t('botComparison.botOfflineDesc') }}
        </div>
      ...
      <div
        v-if="botState"
        class="w-full pt-2 mt-1 border-t border-gray-300 dark:border-gray-700 text-gray-600 dark:text-gray-500 space-y-0.5"
    ...
    <template v-else>
      <!-- Header -->
      <div
        class="flex items-center justify-between mb-3 pb-2 border-b border-gray-300 dark:border-gray-600"
      >
        <div>
        ...
        <div class="text-right text-[0.85rem] text-gray-600 dark:text-gray-400">
          <div v-tooltip.top="t('tooltips.totalTradeCount')">
            {{ profit?.trade_count ?? 0 }} trades
          </div>
      ...
      <div
        v-else-if="botId && closedTrades.length === 0 && hasTradeData"
        class="mb-2 text-center text-gray-600 dark:text-gray-500"
        style="font-size: 0.9rem"
      >
          ...
          <div class="flex-1">
            <div class="mb-1.5">
              <div class="flex justify-between items-baseline">
                <span class="text-gray-600 dark:text-gray-400" v-tooltip.top="t('tooltips.roi')"
                  >ROI</span
                >
            ...
            <div class="space-y-0.5">
              <div class="flex justify-between">
                <span
                  ...
                  v-tooltip.top="t('tooltips.profitFactor')"
                  >{{ t('profit.profitFactor') }}</span
                >
                <span>{{
                  profit?.profit_factor ? formatNumber(profit.profit_factor, 2) : 'N/A'
                }}</span>
              </div>
              <div class="flex justify-between">
                <span
                  class="text-gray-600 dark:text-gray-400"
                  v-tooltip.top="t('tooltips.roiAll')"
                  >{{ t('profit.roiClosed') }}</span
                >
              ...
              <div v-if="(profit?.capital_withdrawal ?? 0) > 0" class="flex justify-between">
                <span
                  class="text-gray-600 dark:text-gray-400"
                  v-tooltip.top="t('tooltips.withdrawn')"
                  >{{ t('profit.withdrawn') }}</span
                >
                <span class="text-yellow-400"
                  >-{{ formatPriceCurrency(profit?.capital_withdrawal ?? 0, currency, 2) }}</span
                >
              </div>
              <div
                v-if="(profit?.capital_withdrawal ?? 0) > 0"
                class="flex justify-between font-bold"
              >
                <span
                  class="text-gray-600 dark:text-gray-400"
                  v-tooltip.top="t('tooltips.netProfit')"
                  >{{ t('profit.netProfit') }}</span
                >
                <span
                  :class="
                    metricColor(
                      (profit?.profit_closed_coin ?? 0) - (profit?.capital_withdrawal ?? 0),
                    )
                  "
                >
                  {{
          ...
          <div class="grid grid-cols-3 gap-x-3 gap-y-0.5">
            <div class="flex flex-col items-center">
              <span
                ...
                v-tooltip.top="t('tooltips.cagr')"
                >{{ t('profit.cagr') }}</span
              >
              <span class="font-bold" :class="metricColor(profit?.cagr)">{{
                profit?.cagr && Math.abs(profit.cagr) < 100 ? formatPercent(profit.cagr, 1) : 'N/A'
              }}</span>
            </div>
            <div class="flex flex-col items-center">
              <span
                ...
                >{{ t('profit.sharpe') }}</span
              >
              <span class="font-bold" :class="metricColor(profit?.sharpe)">{{
                profit?.sharpe != null && Math.abs(profit.sharpe) < 1000
                  ? formatNumber(profit.sharpe, 2)
                  : 'N/A'
              }}</span>
            </div>
            <div class="flex flex-col items-center">
              <span
                class="text-gray-600 dark:text-gray-500"
                style="font-size: 0.95rem"
                v-tooltip.top="t('tooltips.sortino')"
                >{{ t('profit.sortino') }}</span
              >
              <span class="font-bold" :class="metricColor(profit?.sortino)">{{
                profit?.sortino != null && Math.abs(profit.sortino) < 1000
                  ? formatNumber(profit.sortino, 2)
                  : 'N/A'
              }}</span>
            </div>
            <div class="flex flex-col items-center">
              <span
                class="text-gray-600 dark:text-gray-500"
                style="font-size: 0.95rem"
                v-tooltip.top="t('tooltips.sqn')"
                >{{ t('profit.sqn') }}</span
              >
              <span class="font-bold" :class="metricColor(profit?.sqn)">{{
                profit?.sqn != null && Math.abs(profit.sqn) < 1000
                  ? formatNumber(profit.sqn, 2)
                  : 'N/A'
              }}</span>
            </div>
            <div class="flex flex-col items-center">
              <span
                class="text-gray-600 dark:text-gray-500"
                style="font-size: 0.95rem"
                v-tooltip.top="t('tooltips.expectancy')"
                >{{ t('profit.expectancy') }}</span
              >
            ...
            <div class="flex flex-col items-center">
              <span
                class="text-gray-600 dark:text-gray-500"
                style="font-size: 0.95rem"
                v-tooltip.top="t('tooltips.calmar')"
                >{{ t('profit.calmar') }}</span
              >
              <span class="font-bold" :class="metricColor(profit?.calmar)">{{
                formatNumber(profit?.calmar, 2)
              }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="mb-2 pb-2 border-b border-gray-300 dark:border-gray-700">
          ...
          <div class="space-y-0.5">
            <div class="flex justify-between">
              <span
                class="text-gray-600 dark:text-gray-400"
                v-tooltip.top="t('tooltips.avgDuration')"
                >{{ t('profit.avgDuration') }}</span
              >
            ...
            <div class="flex justify-between">
              <span
                class="text-gray-600 dark:text-gray-400"
                v-tooltip.top="t('tooltips.bestPair')"
                >{{ t('profit.bestPerforming') }}</span
              >
            ...
            <div class="flex justify-between">
              <span
                class="text-gray-600 dark:text-gray-400"
                v-tooltip.top="t('tooltips.tradingVolume')"
                >{{ t('profit.tradingVolume') }}</span
              >
        ...
        <div class="mb-2 pb-2 border-b border-gray-300 dark:border-gray-700">
          ...
          <div class="space-y-0.5 text-gray-600 dark:text-gray-400">
            <div class="flex justify-between">
              ...
              <span>{{
                profit?.max_drawdown_abs
                  ? formatPriceCurrency(profit.max_drawdown_abs, currency, 2)
                  : 'N/A'
              }}</span>
            </div>
            <div class="flex justify-between">
              <span v-tooltip="t('tooltips.currentDrawdown')">{{
                t('profit.currentDrawdown')
              }}</span>
              <span>{{
                profit?.current_drawdown_abs
                  ? formatPriceCurrency(profit.current_drawdown_abs, currency, 2)
                  : 'N/A'
              }}</span>
            </div>
          </div>
        ...
        <div class="text-gray-600 dark:text-gray-500 space-y-0.5" style="font-size: 0.9rem">
          <div class="flex justify-between">
            ...
            <span class="text-gray-700 dark:text-gray-300">{{
              (botState?.strategy as string) || 'N/A'
            }}</span>
          </div>
          <div class="flex justify-between">
            <span>{{ t('botInfoCard.exchange') }}</span>
            <span class="text-gray-700 dark:text-gray-300">{{
              (botState?.exchange as string) || 'N/A'
            }}</span>
          </div>
          <div v-if="port" class="flex justify-between">
            <span>{{ t('botInfoCard.port') }}</span>
          ...
          <div v-if="profit?.bot_start_timestamp" class="flex justify-between">
            ...
            <span class="text-gray-700 dark:text-gray-300">{{
              timestampms(profit.bot_start_timestamp)
            }}</span>
          </div>
        </div>
      </template>
