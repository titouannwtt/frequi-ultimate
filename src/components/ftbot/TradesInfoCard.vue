...
const largestPosition = computed(() => {
  ...
  return openTrades.value.reduce(
    (max, t) => (t.stake_amount > (max?.stake_amount ?? 0) ? t : max),
    openTrades.value[0],
  );
});
...
const winrate = computed(() =>
  totalClosed.value > 0 ? (wins.value / totalClosed.value) * 100 : 0,
);
  ...
  <div class="glass-card" style="width: 540px">
    ...
    <div v-if="!hasAnyTradingActivity" class="flex flex-col items-center justify-center py-8 gap-2">
      ...
      <div class="text-sm text-gray-800 dark:text-gray-200">
        {{ t('emptyStates.noTradingActivity') }}
      </div>
      <div class="text-xs text-gray-600 dark:text-gray-400 text-center">
        {{ t('emptyStates.noTradingActivityDesc') }}
      </div>
    </div>
    ...
    <template v-else>
        ...
        <div class="max-h-none space-y-0.5 custom-scrollbar">
          ...
          >
            ...
            <span
              ...
              style="min-width: 60px; max-width: 75px"
            >
              {{ trade.pair.replace(/\/.*/, '') }}
            </span>
            <!-- Direction badge -->
      ...
      >
        ...
        <div class="space-y-1 mb-2">
          <div class="stat-row">
            <span class="stat-label" v-tooltip.top="t('tooltips.avgClosedDuration')">{{
              t('tradesInfoCard.avgClosedDuration')
            }}</span>
            <span class="stat-value">{{ profit?.avg_duration ?? 'N/A' }}</span>
          </div>
          <div class="stat-row">
            <span class="stat-label" v-tooltip.top="t('tooltips.durationAnomaly')">{{
              t('tradesInfoCard.longestOpen')
            }}</span>
          ...
          <div v-if="avgClosedDurationMs > 0" class="stat-row">
            <span class="stat-label" v-tooltip.top="t('tooltips.anomalyScore')">{{
              t('tradesInfoCard.anomalyScore')
            }}</span>
            <span
              class="stat-value font-bold"
              :class="
                durationAnomalyScore > 2
                  ? 'text-red-400'
                  : durationAnomalyScore > 1.5
                    ? 'text-amber-400'
                    : 'text-green-400'
              "
            >
        ...
        <div class="relative space-y-0.5">
          ...
          >
            <span
              class="text-[0.8rem] text-gray-600 dark:text-gray-500 truncate"
              style="min-width: 42px; max-width: 50px"
            >
              {{ trade.pair.replace(/\/.*/, '') }}
            </span>
            <div class="flex-1 h-1.5 rounded-full bg-white/5 overflow-hidden relative">
              <div
                ...
                :class="
                  tradeDurationStatus(trade) === 'red'
                    ? 'bg-gradient-to-r from-red-600 to-red-400'
                    : tradeDurationStatus(trade) === 'yellow'
      ...
      <div class="mb-3 pb-3 border-b border-black/5 dark:border-white/5">
        ...
        <div class="flex gap-3 mb-2 text-[0.85rem]">
          <div class="flex items-center gap-1">
            ...
            <span class="text-gray-800 dark:text-gray-200">{{
              formatPriceCurrency(winningStake, currency, 0)
            }}</span>
          </div>
          <div class="flex items-center gap-1">
            ...
            <span class="text-gray-800 dark:text-gray-200">{{
              formatPriceCurrency(losingStake, currency, 0)
            }}</span>
          </div>
          <div class="flex items-center gap-1">
        ...
        <div class="space-y-0.5">
          <div class="stat-row">
            <span class="stat-label" v-tooltip="t('tooltips.capitalAllocation')">{{
              t('tradesInfoCard.totalAllocated')
            }}</span>
          ...
          <div class="stat-row">
            <span class="stat-label" v-tooltip="t('tooltips.capitalInTrades')">{{
              t('tradesInfoCard.avgStake')
            }}</span>
            <span class="stat-value">{{
              formatPriceCurrency(avgStakePerPosition, currency, 2)
            }}</span>
          </div>
          <div v-if="largestPosition" class="stat-row">
            <span class="stat-label" v-tooltip="t('tooltips.totalStakeLocked')">{{
              t('tradesInfoCard.largestPosition')
            }}</span>
            <span class="stat-value">
              {{ largestPosition.pair.replace(/\/.*/, '') }}
              <span class="text-gray-600 dark:text-gray-500 ml-1">{{
                formatPriceCurrency(largestPosition.stake_amount, currency, 0)
              }}</span>
            </span>
          </div>
        </div>
      ...
      <div class="mb-3 pb-3 border-b border-black/5 dark:border-white/5">
        ...
        <div class="space-y-0.5">
          <div v-if="tradesPerDay !== undefined" class="stat-row">
            <span class="stat-label" v-tooltip="t('tooltips.tradesPerDay')">{{
              t('tradesInfoCard.perDay')
            }}</span>
          ...
          <div v-if="tradesPerWeek !== undefined" class="stat-row">
            <span class="stat-label" v-tooltip="t('tooltips.tradesPerWeek')">{{
              t('tradesInfoCard.perWeek')
            }}</span>
            <span class="stat-value">{{ formatNumber(tradesPerWeek, 1) }}</span>
          </div>
          <div v-if="avgTimeBetweenTradesMs > 0" class="stat-row">
            <span class="stat-label" v-tooltip="t('tooltips.timeBetweenTrades')">{{
              t('tradesInfoCard.avgTimeBetween')
            }}</span>
            <span class="stat-value">{{ humanDuration(avgTimeBetweenTradesMs) }}</span>
          </div>
          <div v-if="latestTradeTs > 0" class="stat-row">
            <span class="stat-label" v-tooltip="t('tooltips.timeSinceLastTrade')">{{
              t('tradesInfoCard.sinceLastTrade')
            }}</span>
      ...
      <div>
        ...
        <div class="flex justify-between">
          <span class="text-[0.85rem]">
            ...
            <span class="text-gray-600 dark:text-gray-500 ml-0.5">{{
              t('tradesInfoCard.wins')
            }}</span>
          </span>
          <span class="text-[0.85rem]">
            <span class="text-red-400 font-bold">{{ losses }}</span>
            <span class="text-gray-600 dark:text-gray-500 ml-0.5">{{
              t('tradesInfoCard.losses')
            }}</span>
          </span>
