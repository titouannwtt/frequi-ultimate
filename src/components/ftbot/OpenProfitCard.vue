...
const botBalance = computed(
  () => botStore.allBalance[props.botId]?.total_bot ?? botStore.allBalance[props.botId]?.total ?? 0,
);
const pctOfBotBalance = computed(() =>
  botBalance.value > 0 ? (totalStakeAllocated.value / botBalance.value) * 100 : 0,
);
...
const pctOfAllBotsStake = computed(() =>
  totalAllBotsOpenStake.value > 0
    ? (totalStakeAllocated.value / totalAllBotsOpenStake.value) * 100
    : 0,
);
...
const highestGain = computed(() => {
  ...
  return openTrades.value.reduce(
    (best, t) => ((t.profit_ratio ?? 0) > (best.profit_ratio ?? 0) ? t : best),
    openTrades.value[0],
  );
});
...
const highestLoss = computed(() => {
  if (openTrades.value.length === 0) return null;
  return openTrades.value.reduce(
    (worst, t) => ((t.profit_ratio ?? 0) < (worst.profit_ratio ?? 0) ? t : worst),
    openTrades.value[0],
  );
});
...
const avgProfit = computed(() => {
  if (openTrades.value.length === 0) return 0;
  return (
    openTrades.value.reduce((sum, t) => sum + (t.profit_ratio ?? 0), 0) / openTrades.value.length
  );
});
...
const weightedAvgProfit = computed(() => {
  ...
  return (
    openTrades.value.reduce((sum, t) => sum + (t.profit_ratio ?? 0) * (t.stake_amount ?? 0), 0) /
    totalStakeAllocated.value
  );
});
...
const profitDistribution = computed(() => {
  if (openTrades.value.length === 0) return [];
  const sorted = [...openTrades.value].sort(
    (a, b) => (a.profit_ratio ?? 0) - (b.profit_ratio ?? 0),
  );
  ...
  <div class="glass-card" style="width: 540px">
    ...
    >
      ...
      <div class="text-sm text-gray-800 dark:text-gray-200">
        {{ t('emptyStates.noOpenPositions') }}
      </div>
      <div class="text-xs text-gray-600 dark:text-gray-400 text-center">
        {{ t('emptyStates.noOpenPositionsDesc') }}
      </div>
    </div>
    ...
    <template v-else>
        ...
        <div class="max-h-none space-y-0.5 custom-scrollbar">
          ...
          >
            ...
            <span
              ...
              :class="trade.is_short ? 'direction-badge--short' : 'direction-badge--long'"
            >
              {{ trade.is_short ? 'S' : 'L' }}
            </span>
            <!-- Leverage -->
            <span
              ...
              class="text-[0.8rem] text-yellow-400 font-bold"
            >
              {{ trade.leverage }}x
            </span>
            <!-- Entry → Current -->
            ...
            <!-- Exit value -->
            <span
              class="ml-1 text-[0.8rem]"
              :class="
                (trade.profit_abs ?? 0) >= 0
            ...
            <!-- Best/Worst badge -->
            <span
              v-if="bestTrade && trade.trade_id === bestTrade.trade_id"
              class="badge-best"
              v-tooltip.top="t('openProfitCard.bestPosition')"
            >
              B
            </span>
            <span
              v-else-if="
                worstTrade &&
                trade.trade_id === worstTrade.trade_id &&
                (worstTrade.profit_ratio ?? 0) < 0
              "
              class="badge-worst"
              v-tooltip.top="t('openProfitCard.worstPosition')"
            >
              W
            </span>
          </div>
      ...
      <div
        v-else
        class="mb-3 pb-3 border-b border-black/5 dark:border-white/5 text-center text-gray-600 dark:text-gray-500 py-3 text-[0.8rem]"
      >
      ...
      >
        ...
        <div class="space-y-0.5">
          <div v-if="highestLoss" class="stat-row">
            <span class="stat-label" v-tooltip.top="t('tooltips.highestLoss')">{{
              t('openProfitCard.highestLoss')
            }}</span>
            <span class="stat-value text-red-400">
              {{ highestLoss.pair.replace(/\/.*/, '') }}
              {{ formatPercent(highestLoss.profit_ratio ?? 0, 1) }}
            </span>
          </div>
          <div v-if="highestGain" class="stat-row">
            <span class="stat-label" v-tooltip.top="t('tooltips.highestGain')">{{
              t('openProfitCard.highestGain')
            }}</span>
            <span class="stat-value text-green-400">
              {{ highestGain.pair.replace(/\/.*/, '') }}
              {{ formatPercent(highestGain.profit_ratio ?? 0, 1) }}
            </span>
          </div>
          <div class="stat-row">
            <span class="stat-label" v-tooltip.top="t('tooltips.avgPositionProfit')">{{
              t('openProfitCard.avgPositionProfit')
            }}</span>
          ...
          <div class="stat-row">
            <span class="stat-label" v-tooltip.top="t('tooltips.weightedAvgProfit')">{{
              t('openProfitCard.weightedAvgProfit')
            }}</span>
          ...
          <div class="stat-row">
            <span class="stat-label" v-tooltip.top="t('tooltips.totalStakeLocked')">{{
              t('openProfitCard.totalStakeLocked')
            }}</span>
            <span class="stat-value">{{
              formatPriceCurrency(totalStakeAllocated, currency, 2)
            }}</span>
          </div>
          <div class="stat-row">
            <span class="stat-label" v-tooltip.top="t('tooltips.pctOfBotBalance')">{{
              t('summaryCards.pctOfBotBalance')
            }}</span>
            <span class="stat-value text-blue-400">{{
              formatPercent(pctOfBotBalance / 100, 1)
            }}</span>
          </div>
          <div class="stat-row">
            <span class="stat-label" v-tooltip.top="t('tooltips.pctOfAllBots')">{{
              t('summaryCards.pctOfAllBotsOpenPositions')
            }}</span>
            <span class="stat-value text-blue-300">{{
              formatPercent(pctOfAllBotsStake / 100, 1)
            }}</span>
          </div>
        </div>
      ...
      <div v-if="openTrades.length > 0">
        ...
        <div class="space-y-0.5">
          <div class="stat-row">
            <span class="stat-label" v-tooltip="t('tooltips.durationAnomaly')">{{
              t('openProfitCard.avgOpenDuration')
            }}</span>
            <span class="stat-value">{{ humanDuration(avgOpenDurationMs) }}</span>
          </div>
          <div class="stat-row">
            <span class="stat-label" v-tooltip="t('tooltips.avgClosedDuration')">{{
              t('openProfitCard.avgClosedDuration')
            }}</span>
          ...
          <template v-if="historicalDurations">
            <div class="stat-row">
              <span class="stat-label" v-tooltip.top="t('tooltips.historicalRange')">{{
                t('openProfitCard.historicalRange')
              }}</span>
              <span class="stat-value"
                >{{ humanDuration(historicalDurations.min) }} —
                {{ humanDuration(historicalDurations.max) }}</span
              >
            </div>
            <div class="stat-row">
              <span class="stat-label" v-tooltip.top="t('tooltips.medianDuration')">{{
                t('openProfitCard.median')
              }}</span>
            ...
            <div
              v-for="trade in sortedTrades"
              :key="'hist-' + trade.trade_id"
              class="flex items-center gap-1.5 mt-1"
            >
              <span
                class="text-[0.85rem] text-gray-600 dark:text-gray-400 truncate"
              ...
              <div class="flex-1 h-1.5 rounded-full bg-white/5 overflow-hidden relative">
                <div
                  ...
                  :class="
                    durationBarPercent(trade) > 100
                      ? 'bg-red-500'
                      : durationBarPercent(trade) > 75
                        ? 'bg-amber-500'
                        : 'bg-blue-500'
                  "
                ...
                <div
                  ...
                  :style="{
                    left: `${((historicalDurations.median - historicalDurations.min) / (historicalDurations.max - historicalDurations.min)) * 100}%`,
                  }"
                />
              </div>
              <span
                class="text-[0.8rem] text-gray-600 dark:text-gray-500"
