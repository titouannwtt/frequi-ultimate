export default {
  ...
  dashboard: {
    ...
    botComparison: 'Global · Bot comparison',
    openTrades: 'Bot · Open trades',
    openTradesDesc:
      'Open trades of all selected bots. Click on a trade to go to the trade page for that trade/bot.',
    closedTrades: 'Bot · Closed trades',
    closedTradesDesc:
      'Closed trades for all selected bots. Click on a trade to go to the trade page for that trade/bot.',
    profitBenchmark: 'Bot · Profit & benchmarks',
    cumulativeProfit: 'Cumulative Profit',
    profitDistribution: 'Bot · Profit distribution',
    tradesLog: 'Trades Log',
    activityTimeline: 'Global · Activity timeline',
    marketOverview: 'Global · Market pulse',
    performanceHeatmap: 'Performance Heatmap',
    riskOverview: 'Global · Risk overview',
    stressTest: 'Global · Stress test',
    logConsole: 'System · Log console',
    rateBudget: 'Rate Budget',
    ratePulse: 'Rate Pulse',

    cacheHealth: 'Cache Health',
    rateMonitor: 'System · API budget',
    requestTimeline: 'System · API requests',
    infraHealth: 'System · Infrastructure',
    volumeComparator: 'Global · Volume comparator',
    periodBreakdown: 'Global · Period breakdown',
    botProfitComparison: 'Bot · Profit per bot',
    botComparisonDesc:
      'Health and performance of every connected bot. The checkboxes select which bots feed the "Selection" widgets.',
    ...
    logConsoleDesc: 'Aggregated bot logs (warnings and errors). Refreshes every 10s.',
    rateMonitorDesc: 'Exchange API budget: request rate, cache hit ratio, 429 errors.',
    requestTimelineDesc: 'Timeline of API requests per bot: direct, cached, errors, latency.',
    infraHealthDesc: 'Bot process health: state, uptime, heartbeats, pairlist size.',
    volumeComparatorDesc: 'Bot traded volume versus exchange volume per period.',
    periodBreakdownDesc: 'Profit broken down by day, week or month across all bots.',
    botProfitComparisonDesc:
      'Profit contribution per bot over the chosen period, for the selected bots.',
    opened: 'opened',
    closed: 'closed',
  },
  widgetScope: {
    selection: 'Selection',
    selectionTitle: 'Data from the bots currently selected in the bot comparison widget',
    all: 'All bots',
    allTitle: 'Data from all connected bots, regardless of selection',
    fleet: 'Fleet',
    fleetTitle:
      'Server-side aggregate over the entire fleet, including bots not connected in this browser',
    system: 'System',
    systemTitle: 'Infrastructure and API monitoring, not trading data',
  },
  widgetState: {
    retry: 'Retry',
  ...
  botProfitComparison: {
    ...
    bots: 'Bots',
    trades: 'Trades',
    tradeCount: '{n} trades in period',
    sortBy: 'Sort by',
    sortProfit: 'Profit',
    sortMagnitude: 'Magnitude',
    sortName: 'Name',
    exportPNG: 'Export as image',
    noData: 'No closed trades for the selected bots and period',
  },
  volumeComparator: {
  ...
  botComparison: {
    ...
    sortByCurrentProfit: 'Current Profit',
    ...
    dryRunReplay: 'Simulate dry-run (replay)',
    dryRunReplayView: 'View running dry-run replay',
    dryRunReplayDone: 'A dry-run replay was already simulated. Reset the dry-run DB to re-run.',
    ...
    replay: {
      ...
      intro:
        'Replays the bot’s real live loop over historical data, then seeds the simulated trades into this bot’s dry-run database — as if it had been running since the chosen date.',
      strategy: 'Strategy',
      wallet: 'Simulated wallet',
      pairs: 'Pairs (current filter)',
      loading: 'Loading current pairs…',
      pairsWarning:
        'The replay uses the {n} pairs currently selected by the filter. Dynamic pairlists are not replayed historically → selection bias on these specific pairs.',
      noPairs:
      ...
      preventDry: 'Dry-run only — never places real orders.',
      preventIsolated: 'Runs in an isolated background process — the bot keeps trading.',
      preventOnce: 'One-shot: to re-run, reset the dry-run database.',
      alreadyTitle: 'Already simulated',
      alreadyMsg:
        'A dry-run replay was already seeded on this bot ({timerange}). To run a new one, start from a fresh dry-run database.',
      resultTitle: 'Result (simulated)',
      closed: 'Closed trades',
      winRate: 'Win rate',
      pnl: 'Total P&L',
      doneReload: 'Done — bot reloaded with the simulated history.',
      run: 'Run replay',
      close: 'Close',
      resolution: 'Resolution (sub-step)',
      resolutionHint:
        'Smaller sub-step = slower but more faithful (finer intra-candle order fills). 1m needs 1m data downloaded.',
      today: 'Today',
      stop: 'Stop',
      elapsed: 'Elapsed',
      eta: 'ETA',
      runningTitle: 'Dry-run replay in progress',
      seededTitle: 'Seeded by a dry-run replay ({timerange}) — click to view / re-seed',
      seededTitleShort: 'Seeded by a dry-run replay — click to view / re-seed',
      badge: 'Dry-run replay',
      pausedBadge: 'Replay paused',
      coverageLoading: 'Checking local data…',
      coverageNone:
        'No {tf} data found locally for these pairs — fills at this resolution won’t be available.',
      coverageEarliest:
        'Earliest {tf} data: {date} ({n}/{total} pairs). A replay before this date won’t be faithful at this resolution.',
      startBeforeData: 'No history before {date}; the replay will only be faithful from that date.',
      endFuture:
        'End date is in the future — the replay stops at the latest available data (downloadable up to recent on Hyperliquid).',
      closeContinues:
        'You can close this window — the replay keeps running in the background. A spinner on the bot in “Bot comparison” shows its progress.',
      restore: 'Restore DB',
      restoreHint:
        'Restore the database snapshot taken just before this replay (undo a crashed or unwanted seed). The bot reloads.',
      restoreConfirm:
        'Restore this bot’s database from the pre-replay backup? The current (seeded) database will be overwritten and the bot will reload.',
      tradeBadge: 'replay',
      resetDb: 'Reset the database first',
      resetDbHint:
        'Wipe existing dry-run trades before seeding. Unchecked: existing trades are kept and the replay stops at the first real trade.',
      detailPeriod: 'Period',
      detailDuration: 'Took',
      detailPairs: 'Pairs',
      detailReplayResult: 'Replay result',
      detailCurrent: 'Current (replay + live-dry since)',
      detailOutside: 'Trades since the replay',
      queueTitle: 'Coordinator: {cap} free slots ({cores} cores, {hopt} used by hyperopt)',
      prioritize: 'Prioritize',
      stateQueued: 'Queued — waiting for a free core',
      statePaused: 'Paused — cores busy (hyperopt or higher-priority replays)',
      tradeBadgeHint: 'Simulated trade seeded by a dry-run replay (not live activity).',
    },
  },

  maxDrawdownCard: {
    title: 'Max Drawdown',
    none: 'No drawdown yet',
    noneDesc: 'No peak-to-trough decline recorded.',
    realized: 'Realized (closed)',
    inclOpen: 'Incl. open positions',
    realizedShort: 'Real.',
    openShort: 'Open',
    ...
    peak: 'Peak',
    trough: 'Trough',
    openPositions: 'Open positions (now → lowest)',
    low: 'low',
    responsibleTrades: 'Responsible trades',
    tradesInWindow: 'Trades in the drawdown window',
    group: 'Group',
    summary: 'Summary',
  },
  ...
  stressTest: {
    ...
    noOpenPositionsHint: 'The scenario will apply as soon as a bot opens a trade',
    isolatedOnly:
  ...
  riskOverview: {
    ...
    riskHigh: 'High Risk',
    noOpenPositions: 'No open positions',
    noOpenPositionsHint: 'Risk metrics will appear as soon as a bot opens a trade',
  },

  // Fleet view
  fleet: {
    title: 'Fleet',
    servedBy: 'served by {bot}',
    refresh: 'Refresh',
    loading: 'Loading fleet data...',
    walletReconciliation: 'Fleet · Wallet reconciliation',
    allClear: 'all clear',
    issueCount: '{count} issue(s)',
    directionalExposure: 'Fleet · Directional exposure',
    pnlContribution: 'Fleet · P&L contribution',
    botsPanel: 'Fleet · Bots',
    reconciliationDesc:
      'Compares on-chain wallet positions with the sum of all bot databases. Server-side aggregate, refreshes every 60s.',
    exposureDesc:
      'Net long/short exposure of the whole fleet. Server-side aggregate, refreshes every 60s.',
    pnlDesc:
      'P&L contribution per bot across the whole fleet. Server-side aggregate, refreshes every 60s.',
    botsDesc:
      'All fleet bots with capital, P&L and activity. Server-side aggregate, refreshes every 60s.',
    errors: {
      overviewFailed: 'Failed to load fleet overview',
      reconciliationFailed: 'Failed to load reconciliation',
      noServingBot:
        'No connected bot supports FleetView. Restart at least one bot with the updated backend.',
      servingBotUnavailable: 'FleetView serving bot unavailable.',
      resolveFailed: 'Resolve request failed',
    },
    botsTable: {
      botCount: '{count} bots',
      capital: 'Capital',
      ...
      total: 'Total',
      open: 'Open',
      filterPlaceholder: 'Filter bot / strategy',
      modeLive: 'Live',
      modeDry: 'Dry',
      modeAll: 'All',
      colBot: 'Bot',
      ...
      colPort: 'Port',
      dryTag: 'DRY',
      frozenTitle: 'Frozen (effective capital <= 0)',
      inertTitle: 'Inert (no trade in 14d)',
      leverageTitle: 'Leverage {leverage}x',
      dbUnreadable: 'Database unreadable',
      durationM: '{m}m',
      durationHM: '{h}h {m}m',
      durationDH: '{d}d {h}h',
    },
    pnl: {
      includeUpnl: 'include uPnL (approx)',
      sum: 'Sum:',
      nonZeroBots: '{count} bots with non-zero contribution',
      noContribution: 'No contribution in this window.',
      window1d: '1d',
      window7d: '7d',
      window30d: '30d',
    },
    exposure: {
      ...
      longWord: 'long',
      shortWord: 'short',
      allocatedStake: '(allocated stake, {total} USDC)',
      topNet: 'Top net on-chain exposures',
      walletNet: 'Wallet net:',
      upnlTitle: 'uPnL {upnl} USDC',
      noPosition: 'No on-chain position.',
    },
    reconciliation: {
      liveBots: '{count} live bots',
      ...
      sideShort: 'short',
      summaryMinority:
        'DB and wallet agree on the total ({chainSide} {chainAmount} {coin}), but {bot} #{tradeId} holds the opposite side of the net position: its exits are rejected as reduce-only.',
      summaryPhantom:
        'The share of {bot} #{tradeId} does not exist on-chain: the wallet holds {chainSide} {chainAmount} {coin}, fully accounted for by the other bots. Deleting it from the DB is safe (no exchange order).',
      summaryUnowned:
        'The wallet holds {chainSide} {chainAmount} {coin} but no bot claims this position.',
      summaryAmbiguous:
        'Bots collectively claim {dbSide} {dbAmount} {coin} but wallet holds {chainSide} {chainAmount} (gap {gap}). No single trade explains the gap: see the resolution guide.',
      status: {
        phantom: 'phantom',
        minority: 'minority',
        unowned: 'unowned',
      ...
      hint: {
        phantom:
          'A bot holds a trade whose share is provably absent on-chain. Safe to delete from its DB (no exchange order).',
        minority:
          'DB matches on-chain, but one bot holds the OPPOSITE side of the net position. Its exits are rejected (reduce-only).',
        unowned:
          'On-chain position with no owning bot in any DB. Can be flattened with a reduce-only order.',
        ambiguous:
          'DB and on-chain disagree with no single provable cause. Manual investigation required.',
      },
      colBot: 'Bot',
      colTrade: 'Trade',
      colSignedAmount: 'Signed amount',
      colOpenRate: 'Open rate',
      colStake: 'Stake',
      closeMinority: 'Close minority slice',
      deletePhantom: 'Delete phantom trade ({bot} #{tradeId})',
      flattenReduceOnly: 'Flatten (reduce-only)',
      manualOnly: 'Manual intervention required, no automatic action available.',
      allInSync: 'DB and on-chain positions are in sync.',
      coinsInSync: '{count} coins in sync',
      guide: {
        title: 'Resolution guide',
        refreshNow: 'Refresh now',
        refreshFirst:
          'Refresh twice ~1 min apart. If the gap moves or disappears it was an in-flight order: do nothing.',
        dust: 'Gap is dust (~{notional} USDC). Safe to ignore. Optionally flatten manually with a reduce-only order.',
        multiPhantom:
          "{count} trades are each a possible phantom. Cross-check each bot's exchange fill history: the real phantom is the one whose side is absent on-chain. Delete only that one (DB delete, no order).",
        multiPhantomItem: '{bot} #{tradeId} ({amount})',
        partialFill:
          "Likely unrecorded partial fill: {bot} trade #{tradeId} recorded {dbAmount} but on-chain math implies only {impliedFill} was actually filled. Check the trade's exchange fills, then fix the DB amount, or apply the minority playbook (stop bot, inverse order without reduce-only, close trade in DB, relaunch).",
        staleDb:
          "Nothing reconciles: probably a stopped bot whose DB still holds open trades (not visible to this scan). Check recently stopped bots' databases before touching anything.",
        footer:
          'Never touch strategies or leverage to fix netting issues. Re-check with a forced refresh after any action.',
      },
      dialog: {
        deletePhantomTitle: 'Delete phantom trade on {coin}',
        closeMinorityTitle: 'Close minority slice on {coin}',
        flattenUnownedTitle: 'Flatten unowned position on {coin}',
        warnDeletePhantom:
          'The trade will be removed from the bot database. No exchange order is placed.',
        warnCloseMinority:
          'A market order WITHOUT reduce-only will be placed to realign the wallet, then the stuck trade is deleted from the bot database. The P&L of this trade will NOT be recorded in the bot statistics.',
        warnFlattenUnowned:
          'A reduce-only market order will be placed to flatten the on-chain position. Reduce-only can only shrink the position, never flip it.',
        coin: 'Coin',
        bot: 'Bot',
        trade: 'Trade',
        amount: 'Amount',
        estNotional: 'Est. notional',
        done: 'Done',
        orderResult: "Order {id}: filled {filled} {'@'} {avg} ({notional} USDC)",
        dbDelete: 'DB delete:',
        close: 'Close',
        cancel: 'Cancel',
        confirm: 'Confirm',
      ...
      realign: {
        button: 'Realign DB',
        title: 'Realign DB on on-chain, {coin}',
        dbOnly:
          'DB-only action: no exchange order will ever be placed. Only the bot databases are rewritten to match the on-chain position.',
        intro:
          'Choose what to do with each DB share so their sum matches the on-chain net position. At most one share can be adjusted; closed shares are removed entirely.',
        colAction: 'Action',
        colNewAmount: 'New amount',
        keep: 'Keep',
        closeOp: 'Close',
        adjustOp: 'Adjust',
        onlyOneAdjust: 'Only one share can be adjusted per plan.',
        resulting: 'Resulting sum:',
        target: 'On-chain target:',
        withinTolerance: 'match (2%)',
        outsideTolerance: 'no match',
        previewBtn: 'Preview',
        colBefore: 'Before',
        colAfter: 'After',
        colDelta: 'Delta',
        colNotional: 'Delta notional',
        touchedNotional: 'Touched notional: {notional} USDC',
        backBtn: 'Back',
        applyBtn: 'Apply (DB only)',
  ...
  botList: {
    ...
    emptyState: 'No bots connected yet. Add one to start trading.',
    viewFlat: 'Switch to flat list',
    viewGrouped: 'Group by folder',
    noMatches: 'No bots match your filters.',
    autoRefresh: 'Auto-refresh',
    onlineSince: 'up {duration}',
    dry: 'Dry',
    live: 'Live',
    dryTooltip: 'Dry-run mode — simulated trades, no real money.',
    liveTooltip: 'Live mode — trading with real funds.',
    spotTooltip: 'Spot: buy and sell only, no leverage or shorting.',
    futuresTooltip: 'Futures: supports short, long, and leverage trading.',
    sort: {
      manual: 'Manual order',
      name: 'Name (A→Z)',
      oldest: 'Oldest first',
    ...
    emptyFolder: 'Empty — move a bot here.',
  },
  ...
  boot: {
    connecting: 'Connecting to bots…',
    botsLoaded: '{loaded} / {total} bots loaded…',
